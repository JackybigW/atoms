...
from openmanus_runtime.agent.react import ReActAgent
from openmanus_runtime.exceptions import TokenLimitExceeded
from openmanus_runtime.logger import logger
from openmanus_runtime.prompt.toolcall import NEXT_STEP_PROMPT, SYSTEM_PROMPT
from openmanus_runtime.schema import TOOL_CHOICE_TYPE, AgentState, Message, ToolCall, ToolChoice
from openmanus_runtime.tool import CreateChatCompletion, Terminate, ToolCollection
...
class ToolCallAgent(ReActAgent):
    ...
    async def think(self) -> bool:
        ...
        thinking = response.thinking if response and response.thinking else None
        ...
        try:
            ...
            if self.tool_choices == ToolChoice.NONE:
                ...
                if content:
                    self.memory.add_message(
                        Message.assistant_message(content, thinking=thinking)
                    )
                    return True
                return False
            ...
            assistant_msg = (
                ...
                if self.tool_calls
                else Message.assistant_message(content, thinking=thinking)
            )
