...
from openmanus_runtime.exceptions import TokenLimitExceeded
from openmanus_runtime.logger import logger  # Assuming a logger is set up in your app
from openmanus_runtime.schema import (
    ROLE_VALUES,
    AssistantResponse,
    TOOL_CHOICE_TYPE,
    TOOL_CHOICE_VALUES,
...
def split_thinking_content(text: Optional[str]) -> tuple[Optional[str], str]:
    ...
    match = THINKING_BLOCK_RE.match(text)
    if not match:
        return None, text
...
class LLM:
    ...
    def __init__(
        ...
        if not hasattr(self, "client"):  # Only initialize if not already initialized
            ...
            # cannot reach its CDN (e.g. network-restricted servers in China).
            try:
                try:
                ...
                logger.warning("tiktoken unavailable, using character-based token estimator")
            ...
            if self.api_type == "azure":
                ...
                from openmanus_runtime.bedrock import BedrockClient
    ...
    async def ask(
        ...
        try:
            ...
            if not stream:
                ...
                _thinking, content = split_thinking_content(
            ...
            _thinking, visible_response = split_thinking_content(full_response)
    ...
    async def ask_tool(
        ...
        **kwargs,
        ...
        Returns:
            AssistantResponse: Sanitized visible content, extracted thinking, and tool calls
